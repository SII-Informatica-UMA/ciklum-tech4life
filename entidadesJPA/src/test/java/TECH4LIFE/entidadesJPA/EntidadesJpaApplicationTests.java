...
public class EntidadesJpaApplicationTests {
	...
	//post de mensajes:
	private RequestEntity<MensajeNuevoDTO> post(String scheme, String host, int port, String path, Map<String, String> queryParams, String token, MensajeNuevoDTO body) {
	...
	private RequestEntity<Void> get(String scheme, String host, int port, String path, Map<String, String> queryParams,
			String token) {
		URI uri = uri(scheme, host, port, queryParams, path);
	...
	private <T> RequestEntity<T> post(String scheme, String host, int port, String path,
			Map<String, String> queryParams, T object, String token) {
		URI uri = uri(scheme, host, port, queryParams, path);
		var peticion = RequestEntity.post(uri)
				.header("Authorization", "Bearer " + token)
	...
	private <T> RequestEntity<T> put(String scheme, String host, int port, String path, Map<String, String> queryParams,
			T object, String token) {
		URI uri = uri(scheme, host, port, queryParams, path);
	...
	@Nested
	@DisplayName("En cuando Centros. Cuando Acceso con gerente y/o administrador")
	public class PruebasCentroAdminGerente {
		@Nested
		...
		public class PruebasMensajes {
			...
			public class ListaMensajesVacia {
				...
				public void InsertaMensaje() {
					...
					var peticion = post("http", "localhost", port, "/mensaje/centro", params, token, mensajeNuevo);
					ResponseEntity<MensajeDTO> respuesta = restTemplate.exchange(peticion, MensajeDTO.class);
					assertThat(respuesta.getStatusCode().value()).isEqualTo(201);
				}
			...
			public class ListaMensajesLlena {
				...
				public void devuelveListaMensajes() {
					...
					ResponseEntity<List<MensajeDTO>> responseEntity = restTemplate.exchange(
						peticion, new ParameterizedTypeReference<List<MensajeDTO>>() {});
